...
export default function DrawGuessPage() {
  ...
  if (!authUser) return <LoginRequired icon="Pencil" />;

  if (active) {
    return (
      <Room code={active} userId={authUser.id} userName={authUser.name} avatarUrl={authUser.avatarUrl} onLeave={() => setActive(null)} />
    );
  }
